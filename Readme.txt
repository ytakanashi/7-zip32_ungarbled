...
●説明
 ...
 またMLangにより文字コードの自動選択や変換を行うように修正したものです。

 このバージョンは7-Zip 15.06 betaと7-zip32.dll Ver.9.22.00.01をベースにしています。
...
●インストール
 ...
 1.「7z1506001b_ungarbled.zip」を適当なディレクトリに解凍して下さい。
...
●ビルド方法(開発者向け)
 ・7-Zipは15.06 beta、7-zip32.dllはVer.9.22.00.01、
   コンパイラはMicrosoft Visual C++ 2015を想定しています。
 1.「7z1506-src.7z」( http://sourceforge.net/projects/sevenzip/files/ )をダウンロード、
   「7z1506」ディレクトリを作成し、その中に解凍します。
 2.「7-zip32.dll」「7-zip64.dll」をビルドする場合、
    1.「7z1506/CPP/7zip/Bundles」に「7-zip32」ディレクトリを作成します。
    2.「7z922001.zip」( http://akky.xrea.jp/ )をダウンロード、解凍します。
    3.「src.7z」を解凍し、作成した「7-zip32」ディレクトリにファイルをコピーします。
 3.本ライブラリの「Src.7z」を解凍し、ファイルを「7z1506」に上書きします。
 4.「7-zip32.dll」「7-zip64.dll」は
     「7z1506/CPP/7zip/Bundles/7-zip32/7-zip32.sln」
   「7z.dll」は
     「7z1506/CPP/7zip/Bundles/Format7zF/Format7z.sln」
   を開きます。
...
●注意
 ・MLangにより自動選択された文字コードが正しくない場合、「-mcp」でコードページを指定することができます。
   例: -mcp=51932
       (EUC-JPを指定)
