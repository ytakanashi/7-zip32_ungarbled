...
#include "../../UI/Common/UpdateCallback.h"		// パス変更
...
STDMETHODIMP CArchiveUpdateCallback::GetStream2(UInt32 index, ISequentialInStream **inStream, UInt32 mode)
{
  ...
  *inStream = NULL;
  ...
  RINOK(Callback->GetStream(DirItems->GetLogPath((unsigned)up.DirIndex), isDir, false, mode));	// 削除

  /* 追加ここから */
  const CDirItem &di = DirItems->Items[up.DirIndex];		// 追加
  UString strLogPath = DirItems->GetLogPath(up.DirIndex);
  RINOK(Callback->GetStream(strLogPath, isDir, false, mode));	// 変更
