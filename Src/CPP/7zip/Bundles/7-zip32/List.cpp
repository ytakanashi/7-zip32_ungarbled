...
#include "../../UI/Common/PropIDUtils.h"		// パス変更
...
//  g_StdOut.PrintUString(s, temp);	// 削除
  g_StdOut << s;					// 追加
  ...
  */
  FOR_VECTOR (i, _fields)
  {
    ...
    if (f.PropID == kpidPath)
    {
      if (!techMode)
        g_StdOut << temp;
//      g_StdOut.NormalizePrint_UString_Path(FilePath, TempWString, TempAString);	// 削除
      g_StdOut << FilePath;															// 追加
      if (techMode)
        g_StdOut << MY_ENDL;
      continue;
    }
    ...
    if (f.IsRawProp)
    {
      ...
      if (dataSize != 0)
      {
        ...
        else if (f.PropID == kpidNtReparse)
        {
          ...
          {
            needPrint = false;
//            g_StdOut.PrintUString(s, TempAString);	// 削除
            g_StdOut << s;								// 追加
          }
        }
...
//        g_StdOut.Normalize_UString_Path(TempWString);	// 削除
		g_StdOut <<TempWString; 						// 追加
        else
//          g_StdOut.Normalize_UString(TempWString);	// 削除
		g_StdOut <<TempWString; 						// 追加
        if (techMode)
//          g_StdOut.PrintUString(TempWString, TempAString);	// 削除
          g_StdOut <<TempWString; 								// 追加
        else
...
//      g_StdOut.NormalizePrint_UString_Path(arcPath);	// 削除
      g_StdOut << endl << arcPath;						// 追加
      g_StdOut << endl << endl;
    }
