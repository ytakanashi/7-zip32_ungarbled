    ...
    Int32 *answer)
{
  ...
  dlg.SetFileInfo(newName, newSize, newTime, existName, existSize, existTime);	// 変更(18050002)
  switch(dlg.DoModal())
  {
...
//    _so->PrintUString(_tempU, _tempA); // 削除
    *_so << name; // 追加
    if (position)
      *_so << " <" << *position << ">";
    *_so << endl;
...
//    _so->NormalizePrint_wstr_Path(name);// 削除
    *_so << name;				// 追加
    *_so << endl;
  }
...
HRESULT CExtractCallbackConsole::ExtractResult(HRESULT result)
{
 ...
 削除ここまで */
  ...
  {
//    NumArcsWithError++;										// 削除
//    if (result == E_ABORT										// 削除
//        || result == HRESULT_FROM_WIN32(ERROR_DISK_FULL)		// 削除
//        )														// 削除
//      return result;											// 削除
  /* 追加ここから */
