...
#include "Common/StdOutStream.h"			// 追加
...
#include "../../UI/Common/ExtractingFilePath.h"			// 変更
#include "../../UI/Common/PropIDUtils.h"				// 変更
...
HRESULT CArchiveExtractCallback::GetExtractStream(CMyComPtr<ISequentialOutStream> &outStreamLoc, bool &needExit)
{
 ...
 //   if (!isAnti)				// 削除(19000002)
	if(!_stdOutMode && !isAnti)	// 追加(19000002)
  {
  ...
  else if (!_isSplit)
  {
    ...
    if (g_StdOut.GetProgressDialog())
    {
      ...
      if (prop.vt != VT_EMPTY)									// 変更
          ConvertPropVariantToUInt64(prop, nCompFileSize);		// 変更
      prop.vt = VT_EMPTY;
...
//    LPCWSTR lpMethod = (prop.vt != VT_EMPTY) ? prop.bstrVal : L"";	// 削除(15120002)
      UString s;											// 追加(15120002)
      if (prop.vt != VT_EMPTY)								// 追加(15120002)
          ConvertPropertyToString2(s,prop,kpidMethod);		// 追加(15120002)
      LPCWSTR lpMethod = (!s.IsEmpty()) ? s.Ptr() : L"";	// 追加(15120002)
      g_StdOut.GetProgressDialog()->AddWorkFile(fs2us(processedPath), fs2us(fullProcessedPath), _fi.Attrib, _encrypted, _fi.MTime.FT, _curSize, nCompFileSize, dwCRC, lpMethod);	// 変更
    }
    /* 追加ここまで */
  }
...
/* 追加(19000002)ここから */
    ...
    if (g_StdOut.GetProgressDialog())
    {
      ...
      if (prop.vt == VT_UI4)
      {
        ...
        return E_FAIL;
      RINOK(archive->GetProperty(index, kpidMTime, &prop));// 変更
      RINOK(archive->GetProperty(index, kpidPackSize, &prop));
      UInt64 nCompFileSize = 0;
      if (prop.vt != VT_EMPTY)
      ...
      g_StdOut.GetProgressDialog()->AddSkipFile(fs2us(processedPath), L"", _fi.Attrib, _encrypted, _fi.MTime.FT, _curSize, nCompFileSize, dwCRC, lpMethod);
    }
}
/* 追加(19000002)ここまで*/
...
//  RINOK(_extractCallback2->SetOperationResult(opRes, BoolToInt(_encrypted)))		// 削除
  
  /* 追加 ここから */
  HRESULT res = _extractCallback2->SetOperationResult(opRes, BoolToInt(_encrypted));
  if (res == ERROR_PASSWORD_FILE)
    NFile::NDir::DeleteFileAlways(_diskFilePath);									// 変更
  RINOK(res)
  /* 追加ここまで */
  return S_OK;
