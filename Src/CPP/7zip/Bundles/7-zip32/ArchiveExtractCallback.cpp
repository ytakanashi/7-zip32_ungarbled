...
if (askExtractMode == NArchive::NExtract::NAskMode::kExtract && !_testMode)
{
  ...
  else
  {
    ...
    // ----- Is file (not split) -----
    if (g_StdOut.GetProgressDialog() && _curSizeDefined) g_StdOut.GetProgressDialog()->SeekNextFile(_curSize);	// 追加
...
//    LPCWSTR lpMethod = (prop.vt != VT_EMPTY) ? prop.bstrVal : L"";	// 削除(15120002)
      ...
      if (prop.vt != VT_EMPTY)								// 追加(15120002)
          ConvertPropertyToString(s,prop,kpidMethod);		// 追加(15120002)
