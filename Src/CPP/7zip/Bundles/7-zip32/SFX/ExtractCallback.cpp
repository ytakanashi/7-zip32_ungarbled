    ...
    ISequentialOutStream **outStream, Int32 askExtractMode)
{
  ...
  if (askExtractMode == NArchive::NExtract::NAskMode::kExtract)
  {
    ...
    if (fileInfo.Find(fullProcessedPath))
    {
      ...
      {
        _message = NWindows::MyLoadString(IDS_CANT_DELETE_FILE);	// ïœçX
        return E_FAIL;
      }
    }
    ...
    if (!isAnti)
    {
      ...
      {
        _message = NWindows::MyLoadString(IDS_CANT_OPEN_OUTPUT_FILE);		// ïœçX
        return E_FAIL;
      }
    ...
    default:
    {
      ...
      switch(resultEOperationResult)
      {
        case NArchive::NExtract::NOperationResult::kUnsupportedMethod:
          _message = NWindows::MyLoadString(IDS_UNSUPPORTED_METHOD);	// ïœçX
          break;
        default:
          _isCorrupt = true;
      }
