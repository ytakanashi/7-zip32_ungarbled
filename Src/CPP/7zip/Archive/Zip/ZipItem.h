...
struct CExtraSubBlock
{
  ...
  bool ExtractInfoZipUnicodePath(AString &name, AString &res) const;
};
...
struct CExtraBlock
{
  ...
  bool GetInfoZipUnicodePath(AString &name, AString &res) const
  {
    for (int i = 0; i < SubBlocks.Size(); i++)
    {
      const CExtraSubBlock &sb = SubBlocks[i];
      if (sb.ID == NFileHeader::NExtraID::kInfoZipUnicodePath)
        return sb.ExtractInfoZipUnicodePath(name, res);
    }
    return false;
  }
...
public:
  ...
  {
    UString res;
//    if (IsUtf8()) // Always try to use UTF-8 first
      if (ConvertUTF8ToUnicode(s, res)) {
        if(!IsNormalizedString(NormalizationC, res, res.Length() + 1)) {
          UString src(res);
          int destLen = NormalizeString(NormalizationC, src, src.Length(), NULL, 0);
          wchar_t *p = res.GetBuffer(destLen);
          destLen = NormalizeString(NormalizationC, src, src.Length(), p, destLen);
          if(destLen > 0) {
            p[destLen] = 0;
            res.ReleaseBuffer();
          } else res.Empty();
